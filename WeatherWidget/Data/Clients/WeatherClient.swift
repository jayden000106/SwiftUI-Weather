...
final class WeatherClient {
    ...
    // TODO: UserDefauls나 SwiftData를 통해 실제 앱과 공유
    ...
    func requestLocationRealtime(location: Location) async throws -> RealtimeWeatherDTO {
    ...
    func requestLocationDailyTimelines(location: Location) async throws -> DailyWeatherDTO {
        let geoPoint = "\(location.latitude), \(location.longitude)"
        let result = try await request(
            url: "timelines",
            method: .post,
    ...
    func requestLocationHourlyTimelines(location: Location) async throws -> HourlyWeatherDTO {
        let geoPoint = "\(location.latitude), \(location.longitude)"
        let result = try await request(
            url: "timelines",
            method: .post,
